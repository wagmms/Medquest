...
import { normalizeFlashcard } from '../src/lib/normalizeFlashcard';

test.describe('normalizeFlashcard', () => {
  test('handles missing front and back safely', () => {
  ...
  test('non-legacy cards: strips option letters from cloze in front', () => {
    const card = {
  ...
  test('non-legacy cards: leaves non-option cloze intact', () => {
    const card = {
  ...
  test('legacy cards: parses term and formats correctly', () => {
    const card = {
    ...
    expect(normalized.front).not.toContain('Qual a conduta?');
    expect(normalized.front).toContain('👉 Diagnóstico / Conduta indicada: {{c1::Metformina}}');
    expect(normalized.back).toBe('Gabarito Oficial: Metformina');
  });

  test('legacy cards: parses wrong term from back and adds distractor warning', () => {
    const card = {
    ...
    expect(normalized.front).not.toContain('Qual a conduta mais adequada?');
  ...
  test('legacy cards: handles "Neste caso clínico, em vez de"', () => {
    const card = {
  ...
  test('legacy cards: handles missing stem', () => {
    const card = {
  ...
  test('legacy cards: formats properly even if term is missing', () => {
    const card = {
